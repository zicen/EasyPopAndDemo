package com.zyyoona7.easypopup.panel.view;
...
import com.zyyoona7.easypopup.panel.interfaces.ViewAssertion;
...
public class PanelSwitchLayout extends LinearLayout implements ViewAssertion {
    ...
    public void assertView() {
        if (getChildCount() != 2) {
            throw new RuntimeException("PanelSwitchLayout -- PanelSwitchLayout should has two children,"
                    + "the first is ContentContainer,the other is PanelContainer！");
        }
        View firstView = getChildAt(0);
        View secondView = getChildAt(1);

        if (!(firstView instanceof ContentContainer)) {
            throw new RuntimeException("PanelSwitchLayout -- "
                    + "the first view is ContentContainer,the other is ContentContainer！");
        }
        if (!(secondView instanceof PanelContainer)) {
            throw new RuntimeException(
                    "PanelSwitchLayout -- the second view is ContentContainer,the other is PanelContainer！");
        }
    }
}
