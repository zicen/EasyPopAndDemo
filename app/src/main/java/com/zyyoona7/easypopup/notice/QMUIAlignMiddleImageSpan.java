...
public class QMUIAlignMiddleImageSpan extends ImageSpan {
     ...
     * @param d                 作为 span 的 Drawable
     * @param verticalAlignment 垂直对齐方式, 如果要垂直居中, 则使用 {@link #ALIGN_MIDDLE}
     * @param fontWidthMultiple 设置这个Span占几个中文字的宽度, 当该值 > 0 时, span 的宽度为该值*一个中文字的宽度; 当该值 <= 0 时, span 的宽度由
     *                          {@link #mAvoidSuperChangeFontMetrics} 决定
     */
    ...
    public void draw(@NonNull Canvas canvas, CharSequence text, int start, int end,
        ...
        if (mVerticalAlignment == ALIGN_MIDDLE) {
            ...
            // transY -= (paint.getFontMetricsInt().bottom - paint.getFontMetricsInt().top) / 2 - d
            //         .getBounds().bottom / 2; // 居中对齐
    ...
    /**
     * 是否避免父类修改 FontMetrics，如果为 false 则会走父类的逻辑, 会导致 FontMetrics 被更改
     */
